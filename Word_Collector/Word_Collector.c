/* 
 * App   : Word Collector
 * Author: Jerome Goldwin M., B.tech/IT
 * Date  : Apr 7, 2025
 *
*/
...
int main(){
	...
	// converting upper case letters to smaller case letters
	//
	for(j=0; j<i; j++){
		for(int m=65;m<91;m++){
			if(hash[j]==m)
...
//Function to display the words and how many times it occurred
void display( char s[][30], int i, int *check){
	int j=0;
	printf("%-11s",s[i]);
	printf(" <-- %d times occurred\n",*check);
}
